...
export const sourceModule = (repository, relative) => import(pathToFileURL(path.join(source(repository), relative)).href);
...
export async function run(command, args, options = {}) {
  const child = spawn(command, args, { cwd: root, env: { ...process.env, PATH: `${path.dirname(process.execPath)}:${process.env.PATH ?? ""}` }, stdio: "inherit", ...options });
...
export function platformEnvironment() {
  const result = {};
  for (const name of ["HOME", "PATH", "LANG", "LC_ALL", "TMPDIR", "SYSTEMROOT", "WINDIR", "SSH_AUTH_SOCK"]) {
