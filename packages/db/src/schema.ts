...
import {
  bigint, bigserial, boolean, check, foreignKey, index, integer,
  jsonb, pgSchema, text, timestamp, unique, uniqueIndex,
...
export const accounts = github.table("accounts", {
...
export const repositories = github.table("repositories", {
  id: githubId("id").primaryKey(),
  nodeId: text("node_id").notNull(),
  ownerId: githubId("owner_id").notNull().references(() => accounts.id),
...
export const publicMemberships = github.table("public_memberships", {
  id: bigserial("id", { mode: "bigint" }).primaryKey(),
  personId: githubId("person_id").notNull().references(() => accounts.id),
  organizationId: githubId("organization_id").notNull().references(() => accounts.id),
...
export const pullRequests = github.table("pull_requests", {
  id: githubId("id").primaryKey(),
  nodeId: text("node_id").notNull(),
  repositoryId: githubId("repository_id").notNull().references(() => repositories.id),
...
export const pullRequestReviews = github.table("pull_request_reviews", {
  id: githubId("id").primaryKey(),
  pullRequestId: githubId("pull_request_id").notNull().references(() => pullRequests.id),
...
export const repositoryLanguages = github.table("repository_languages", {
  id: bigserial("id", { mode: "bigint" }).primaryKey(),
  repositoryId: githubId("repository_id").notNull().references(() => repositories.id),
...
export const concepts = github.table("concepts", {
  id: bigserial("id", { mode: "bigint" }).primaryKey(),
...
export const conceptEvidence = github.table("concept_evidence", {
  id: text("id").primaryKey(),
  repositoryId: githubId("repository_id").notNull().references(() => repositories.id),
  pullRequestId: githubId("pull_request_id"),
  conceptId: githubId("concept_id").notNull().references(() => concepts.id),
  sourceKind: text("source_kind", { enum: ["topic", "manifest", "readme", "pull_request"] }).notNull(),
  ...
  reviewStatus: text("review_status", { enum: ["accepted", "pending", "rejected", "superseded"] }).notNull(),
  observedAt: observedAt(),
}, (t) => [
  foreignKey({ columns: [t.pullRequestId, t.repositoryId], foreignColumns: [pullRequests.id, pullRequests.repositoryId], name: "concept_evidence_pull_request_repository_fk" }),
  ...
  check("concept_evidence_source_kind_valid", sql`${t.sourceKind} in ('topic', 'manifest', 'readme', 'pull_request')`),
  ...
  check("concept_evidence_review_valid", sql`${t.reviewStatus} in ('accepted', 'pending', 'rejected', 'superseded')`),
  check("concept_evidence_scope_valid", sql`(${t.sourceKind} = 'pull_request') = (${t.pullRequestId} is not null)`),
]);

export const syncState = ingest.table("sync_state", {
  id: text("id").primaryKey(),
  ...
  check("sync_state_status_valid", sql`${t.status} in ('pending', 'partial', 'complete', 'failed')`),
  check("sync_state_counts_nonnegative", sql`${t.pagesFetched} >= 0 and ${t.recordsFetched} >= 0`),
]);

export const searchRequests = operations.table("search_requests", {
  id: text("id").primaryKey(),
...
}, (t) => [index("search_requests_client_time").on(t.clientHash, t.createdAt), index("search_requests_time").on(t.createdAt)]);
...
export const schema = { accounts, repositories, publicMemberships, pullRequests, pullRequestReviews, repositoryLanguages, concepts, conceptEvidence, syncState, searchRequests, captures };
