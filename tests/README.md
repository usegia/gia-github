...
| `pnpm exec vitest run --project live tests/live/runtime.live.test.ts` | Node 26, bootstrapped pinned Runtime, OpenRouter key, and reachable separate registry database; real-process checks verify unknown profiles fail before signing state is created, and occupied-port startup failure clears its dead receipt while preserving the other TCP server |
