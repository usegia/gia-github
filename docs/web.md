...
The endpoint uses one conservative shared admission bucket, `public-web`. Do not derive a client identity from arbitrary forwarded headers. A deployment that needs per-client budgets must establish its trusted proxy or authenticated identity before changing this boundary. `SEARCH_MAXIMUM_DAILY_SEARCHES` configures the shared daily allowance, defaults to 100, and accepts 1 through 10000. This is an admission count, not a currency budget. Set `APP_ORIGIN` to the public origin, for example `https://people.example.com`, when deploying behind a TLS reverse proxy. The local fallback uses the direct Host and request protocol; forwarded headers are not trusted. The browser does not submit cross-origin searches.
...
`tests/browser/search.spec.ts` uses a running application, PostgreSQL with imported real GitHub data, and an initialized Gia project/profile. Start the production server before `pnpm test:e2e`; Playwright does not own its lifecycle. The full CI workflow starts Next and waits for the catalog endpoint before running the browser suite. Missing prerequisites fail; tests do not intercept or replace API responses.
